...
- Add the packaged JSON Schema Draft 2020-12 resource
  ...
  RFC 9110 token strings and reject `CONNECT`. It requires a non-empty total
  `address_count` plus family-specific IPv4 and IPv6 counts, all derived from
  the same validated addresses so consumers can check their consistency. The
  schema is not an authorization decision or cryptographic signature, and consumers remain
