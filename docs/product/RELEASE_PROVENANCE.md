...
The repository-level verifier is deliberately credential free. Credential-bearing signing, attestation, package publication, release/tag mutation, and any OIDC authority belong to separately reviewed organization workflows and protected environments. A credentialed stage must recheck the exact repository/source identity and every payload digest rather than infer trust from a branch, filename, PR body, or prior workflow status.
