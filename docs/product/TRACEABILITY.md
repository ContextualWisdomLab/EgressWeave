...
| PRD-FR-012 Canonical automation prompt integrity | `.github/prompts/hourly-product-maintainer.md` plus protected-main workflow loader, regular-file/non-symlink check, 12 KiB guard and credential-free verifier boundary | `tests/test_hourly_rca_feasibility_contract.py`, `tests/test_hourly_opencode_nvidia_contract.py`, documentation prompt-governance contracts | **ADR 0004** | IMPLEMENTED-ON-PROTECTED-MAIN |
...
| Bounded canonical prompt and resumable control-plane incident handling | Accepted ADR 0004 plus protected-main canonical prompt, workflow loader, byte guard, authority separation and incident runbook | `tests/test_hourly_rca_feasibility_contract.py`, `tests/test_hourly_opencode_nvidia_contract.py`, `tests/test_documentation_prompt_budget_governance.py` | **ADR 0004** | IMPLEMENTED-ON-PROTECTED-MAIN for repository-owned controls; external scheduler acceptance is separate |
