...
| PRD-FR-012 Canonical automation prompt integrity | `.github/prompts/hourly-product-maintainer.md` plus protected-main workflow loader, regular-file/non-symlink check, exact **12,000-byte** guard, credential-free verifier boundary, and model edit allowlist that denies `.github/**` | `tests/test_hourly_rca_feasibility_contract.py`, `tests/test_hourly_opencode_nvidia_contract.py`, documentation prompt-governance contracts | **ADR 0004** | IMPLEMENTED-ON-PROTECTED-MAIN |
...
| Bounded canonical prompt and resumable control-plane incident handling | Accepted ADR 0004 plus protected-main canonical prompt, workflow loader, exact **12,000-byte** guard, authority separation, `.github/**` edit denial, and incident runbook that routes prompt correction to an external maintainer | `tests/test_hourly_rca_feasibility_contract.py`, `tests/test_hourly_opencode_nvidia_contract.py`, `tests/test_documentation_prompt_budget_governance.py` | **ADR 0004** | IMPLEMENTED-ON-PROTECTED-MAIN for repository-owned controls; external scheduler acceptance is separate |
...
A generic scheduler error is a **control-plane incident**, not product state. The repository-owned canonical prompt source/loader and authority separation are integrated on protected main, but external scheduler recovery is accepted only from exact operational evidence. The repository-local model workflow records evidence and requests **external maintainer** action for prompt or other `.github/**` correction rather than modifying those controls itself. External prompt repair alone never closes the repository work queue.
...
| Prompt/control-plane integrity | canonical prompt path, exact **12,000-byte** guard, deny-by-default model tools, `.github/**` edit denial, non-publishing handoff and exact-identity recovery | scheduler and documentation prompt-governance contracts | ADR 0003; **ADR 0004** |
