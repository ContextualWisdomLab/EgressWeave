...
EgressWeave prevents an application-controlled outbound URL from escaping an explicit hostname and HTTP-method policy through SSRF, DNS rebinding, authority drift, redirects, environment proxies, application-layer tunnelling, or Unix sockets. This document defines the boundary precisely so integrators do not mistake transport safety for complete application authorization.
...
3. authorizes only the policy's normalized HTTP-method allowlist at the transport boundary, defaults to ordinary API methods, and refuses `CONNECT` even when an operator attempts to configure it because a tunnel destination is independent of the validated URL authority;
4. resolves every address returned by the system resolver and rejects the complete target if any address is not globally routable;
5. signs the resulting `ValidatedEgressURL` with a process-local integrity key and revalidates its URL, hostname, port, address shape, signature, and address scope before transport construction;
6. connects only to the validated address set while preserving the original hostname for certificate verification and TLS Server Name Indication;
7. rejects request scheme, user information, hostname, effective-port, method, or caller-supplied SNI drift before the request reaches the connection pool;
8. replaces any caller-supplied `Host` header and binds the forwarded `sni_hostname` extension to the validated authority;
9. disables redirects and environment-derived proxy configuration;
10. refuses Unix-domain sockets; and
11. returns a deny-all transport when client construction receives no non-empty base URL, so missing or optional configuration cannot silently create unrestricted egress.
...
Use a distinct policy for each trust domain and keep both hostname and method allowlists as small as possible. Supply bare hostnames only—never schemes, credentials, ports, paths, wildcards, or IP literals—and construct the policy during application startup so configuration errors stop deployment before traffic is served. Narrow `allowed_methods` to the operations the integration actually needs; do not treat the default set as a substitute for application authorization. Construct clients once per validated authority, close them deterministically, set application-appropriate HTTPX timeouts, and never fall back to an unguarded HTTP client after `EgressNotAllowedError`.
...
Treat changes to `httpx`, `httpcore`, Python URL parsing, IP classification, resolver behavior, local-address policy, or HTTP-method policy as security-sensitive. Re-run the complete transport and validation suite before widening the supported dependency range.
