...
denial crosses the public response boundary. A dependency-controlled key that
raises a custom `BaseException` is also contained behind the generic denial;
direct `KeyboardInterrupt`, `SystemExit`, and `GeneratorExit` are interpreter or
process control flow and propagate rather than being consumed. This prevents
dependency-provided mapping behavior from becoming an implicit extension
mechanism.
